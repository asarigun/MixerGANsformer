...
parser.add_argument('--dis_batch_size', type=int, default=64 , help='Batch size for discriminator.')
...
generator= TransGenerator(depth1=5, depth2=4, depth3=2, initial_size=8, dim=384, heads=4, mlp_ratio=4, drop_rate=0.5)#,device = device)
#generator = MixerGenerator(initial_size=8, chn_dim=1024, patch_size=2, num_blocks=2, tok_hid_dim=256, chn_hid_dim=2048, num_classes=1, p=0.)
generator.to(device)

discriminator = MixerDiscriminator(in_channels=3, img_size=32, chn_dim=1024, patch_size=16, num_blocks=8, tok_hid_dim=256, chn_hid_dim=2048, num_classes=1, p=0.)
#discriminator = TransDiscriminator(diff_aug = args.diff_aug, image_size=32, patch_size=4, input_channel=3, num_classes=1,
